...
Work ongoing:
- There are two simulated loading times: one for the tab container types, and one for the table data.  The tables cannot be rendered until the tabs are rendered, yet if the tabs are rendered too quickly, the tables not rendered, which implies a race condition.  Am working on a refactor.
